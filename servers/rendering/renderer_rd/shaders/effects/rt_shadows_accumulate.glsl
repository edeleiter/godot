/* rt_shadows_accumulate.glsl - TAA accumulation for RT Shadows (R8 format).
 * REQUIRED: neighborhood clamping prevents visually unacceptable shadow ghosting
 * on moving geometry. blend_factor = 0.25 (4-frame accumulation).
 * This shader is mandatory -- do not disable or skip it.
 */
...
layout(push_constant, std430) uniform PushConstant {
	ivec2 screen_size;
	float temporal_blend;               // ~0.25 (4-frame shadow accumulation)
...
void main() {
	...
	// 3x3 neighborhood AABB -- critical for shadow ghosting prevention.
