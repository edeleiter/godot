...
# Build Godot Editor with .NET/C# support (Windows / Git Bash)
# Usage: ./build_dotnet_editor.sh [--clean] [platform] [jobs]
...
echo "=== Building Godot .NET Editor for ${PLATFORM} ==="
...
if [[ "${CLEAN}" == true ]]; then
    ...
    CLEAN_ARGS="platform=${PLATFORM} module_mono_enabled=yes target=editor"
...
# Step 1: Build the editor binary with mono enabled
echo ""
echo "--- Step 1/4: Building editor binary (platform=${PLATFORM}, mono=yes) ---"
SCONS_ARGS="platform=${PLATFORM} module_mono_enabled=yes target=editor num_jobs=${JOBS} scu_build=yes"
if [[ "${PLATFORM}" == "windows" ]]; then
...
# Determine the binary name (mono builds add .mono suffix)
ARCH="x86_64"
BIN="${SCRIPT_DIR}/bin/godot.${PLATFORM}.editor.${ARCH}.mono"
if [[ "${PLATFORM}" == "windows" ]]; then
    BIN="${BIN}.exe"
fi
